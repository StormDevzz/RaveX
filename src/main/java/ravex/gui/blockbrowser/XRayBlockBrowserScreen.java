...
public class XRayBlockBrowserScreen extends Screen {
    ...
    public XRayBlockBrowserScreen(Screen parent) {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean handled) {
        ...
        if (event.x() >= btnX && event.x() <= btnX + btnW && event.y() >= btnY && event.y() <= btnY + btnH) {
            this.minecraft.setScreen(new ravex.gui.clickgui.ClickGUI());
            return true;
        }
        ...
        if (col >= 0 && col < columns && row >= 0) {
            ...
            if (index < filteredBlocksCache.size()) {
                ...
                if (event.x() >= slotLeft && event.x() <= slotLeft + ITEM_SIZE) {
                    BlockCacheEntry entry = filteredBlocksCache.get(index);
                    boolean current = Xray.INSTANCE.isBlockSelected(entry.identifier);
                    Xray.INSTANCE.setBlockSelected(entry.block, !current);
    ...
    private void updateFilter() {
        ...
        if (currentTab == Tab.SELECTED) {
            ...
            for (BlockCacheEntry entry : allBlocksCache) {
                if (Xray.INSTANCE.isBlockSelected(entry.identifier)) {
                    baseList.add(entry);
                }
            }
