...
public class Trails extends Module {
    ...
    private boolean shouldTrack(Entity entity) {
        ...
        if (tridents.getValue() && entity instanceof ThrownTrident) return true;
        if (fireworks.getValue() && entity instanceof FireworkRocketEntity) return true;
        if (potions.getValue() && entity instanceof AbstractThrownPotion) return true;
        if (fireballs.getValue() && entity instanceof AbstractHurtingProjectile) return true;
        if (windCharges.getValue() && entity instanceof AbstractWindCharge) return true;
        if (other.getValue() && entity instanceof Projectile) return true;
