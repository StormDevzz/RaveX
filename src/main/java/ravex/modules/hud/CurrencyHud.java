...
public class CurrencyHud extends HudModule {
    ...
    // Rates in memory (realistic defaults)
    ...
    private double usdToBtc = 0.0000148; // ~67.5k BTC/USD
    ...
    public void render(GuiGraphics graphics, float partialTicks) {
        ...
        // Simulate tiny realistic ticks/fluctuations every 2 seconds
    ...
    private void simulateTicks() {
        // Fluctuate slightly to look active
        double jitter = 1.0 + (Math.random() - 0.5) * 0.0005; // max 0.025%
        ...
        usdToBtc = Math.max(0.000005, Math.min(0.00005, usdToBtc * (1.0 + (Math.random() - 0.5) * 0.001))); // slightly higher volatility
    }
