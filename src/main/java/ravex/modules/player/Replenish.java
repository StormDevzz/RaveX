...
public class Replenish extends Module {
    ...
    static {
        try {
            nativeLibLoaded = NativeLoader.loadLibrary("ravex_replenish");
        } catch (UnsatisfiedLinkError e) {
            System.err.println("[Replenish JNI] Failed to load native library: " + e.getMessage());
        }
    }
    ...
    public void onTick() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (!stack.isEmpty()) {
                ...
                hbIds[i] = rl != null ? rl.toString() : "";
                hbCounts[i] = stack.getCount();
            } else {
                hbIds[i] = "";
                hbCounts[i] = 0;
            }
        ...
        for (int i = 9; i < 36; i++) {
            ...
            if (!stack.isEmpty()) {
                Identifier rl = BuiltInRegistries.ITEM.getKey(stack.getItem());
                invIds[idx] = rl != null ? rl.toString() : "";
                invCounts[idx] = stack.getCount();
            } else {
                invIds[idx] = "";
                invCounts[idx] = 0;
            }
