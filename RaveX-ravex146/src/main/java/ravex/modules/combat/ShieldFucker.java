...
public class ShieldFucker extends Module {
    ...
    static {
        try {
            nativeAvailable = NativeLoader.loadLibrary("ravex_shieldfucker");
        } catch (Throwable t) {
    ...
    private ShieldFucker() {
        super("ShieldFucker", Category.COMBAT);
        addParameter(range);
        addParameter(wallRange);
        addParameter(switchDelay);
        addParameter(attackDelay);
        addParameter(rotateSpeed);
        addParameter(throughWalls);
        addParameter(targetPlayers);
        addParameter(targetMonsters);
        addParameter(onlyAxe);
